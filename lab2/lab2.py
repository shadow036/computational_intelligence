...
if __name__ == '__main__':
    ...
    for N in Ns:
        ...
        for gen in range(GENERATIONS):
            ...
            c_solutions = mutation(c_solutions, mu, [current_best[0][1] < worst_case_fitness/10, worst_case_fitness/10 <= current_best[0][1] < 0, current_best[0][1] >= 0], input_)
            e_solutions = fitness_function(c_solutions, worst_case_fitness, N)
        ...
        for gen in range(GENERATIONS):
            ...
            for o in range(offsprings[1]):
                ...
                c_solutions.append(recombination(p0, p1))
            c_solutions = mutation(c_solutions, 0, [current_best[1][1] < worst_case_fitness/10, worst_case_fitness/10 <= current_best[1][1] < 0, current_best[1][1] >= 0], input_)
            e_solutions = fitness_function(c_solutions, worst_case_fitness, N)
            e_solutions = replacement(e_solutions, mu)
    ...
    for i, N in enumerate(Ns):
        info(f'N: {N} => (fitness "plus" strategy: {results[i][0][1]}, fitness "comma" strategy: {None if results[i][1] == None else results[i][1][1]})')